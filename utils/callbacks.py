...
class EMA(Callback):
    ...
    def __init__(
        self, decay: float, validate_original_weights: bool = False, every_n_steps: int = 1, cpu_offload: bool = False, deep_speed: bool = False
    ):
        if not (0 <= decay <= 1):
    ...
    def on_fit_start(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        trainer.optimizers = [
            EMAOptimizer(
                optim if not self.deep_speed else optim.optimizer,
                device=device,
                decay=self.decay,
