...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # embedding
        enc_out_list = []
        x_list = self.pre_enc(x_list) # trend and seasonal decomposition
        x_mark_list = [x_mark_list[0] for i in range(len(x_list))]
        if x_mark_enc is not None:
            for i, x, x_mark in zip(range(len(x_list)), x_list, x_mark_list):
                enc_out = self.enc_embedding(x[0], x_mark)  # [B,T,C]
